...
@unittest.skipIf(
    is_in_ci(),
    "Mooncake RDMA GPU zero-copy test requires RDMA hardware, skipping in CI",
)
class TestEPDMooncakeRDMAGPUZeroCopy(PDDisaggregationServerBase):
    """Test EPD disaggregation with mooncake GPU→GPU RDMA zero-copy transfer.

    Validates the async VIT forward + GPU buffer pre-allocation +
    GPU-to-GPU RDMA transfer pipeline using MMMU eval (multi-image).
    """
    ...
    def test_mmmu(self):
        """Test MMMU evaluation with mooncake GPU RDMA zero-copy transfer"""
        import glob
        import json
