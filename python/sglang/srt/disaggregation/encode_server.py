...
class MMEncoder:
        ...
        try:
            ...
            async def _run_forward_with_cache():
                try:
                    ...
                    hit_indices = [
                        i for i, e in enumerate(exist_mask) if e
                    ]
                    final_slices = [None] * num_items
                    ...
                    if missing_indices:
                        new_slices = self._encode_missing(
                            mm_feature, mm_inputs, missing_indices,
                            modality, get_feature_fn, grid_thw,
                            keep_on_gpu=True,
                        )
                        if self.rank == 0:
                            for i, idx in enumerate(missing_indices):
                    ...
                    if self.rank == 0 and hit_indices:
                        ...
                        try:
                            ...
                            await asyncio.wait_for(
                                _wait_prefetch(), timeout=60.0
                            )
                            cached_slices = (
                                self.mm_global_cache.get_embeddings(
                                    hit_hashes
                                )
                            )
                            for i, idx in enumerate(hit_indices):
                    ...
                    if prefetch_status.item() == 0 and hit_indices:
                        fallback_slices = self._encode_missing(
                            mm_feature, mm_inputs, hit_indices,
                            modality, get_feature_fn, grid_thw,
                            keep_on_gpu=True,
                        )
                        if self.rank == 0:
                    ...
                    if self.rank == 0:
                        ...
                        all_new_hashes = [
                            mm_hashes[i] for i in missing_indices
                        ]
                        all_new_slices = list(new_slices)
                        if fallback_slices is not None:
                            all_new_hashes += [
                    ...
                    logger.error(
                        f"Global cache + VIT forward failed for "
                        f"{req_id}: {e}"
                    )
                    if self.rank == 0:
    ...
    async def _send(
        ...
        if self.server_args.encoder_transfer_backend == "mooncake":
            ...
            if embedding is None:
                raise InternalError(
                    f"No embedding available for RDMA transfer: {req_id}"
                )
    ...
    async def encode_with_mooncake(
        ...
        try:
            ...
            # All ranks: launch background VIT forward as an asyncio task so
            # /encode can return metadata immediately.  We intentionally do NOT
            # use run_in_executor here: get_feature_fn relies on a session
            # context (CUDA / SGLang inference session) that is bound to the
            # event-loop main thread and is NOT available inside a
            # ThreadPoolExecutor worker thread.
