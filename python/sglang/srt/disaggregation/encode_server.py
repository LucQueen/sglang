...
class MMEncoder:
    def __init__(
        ...
        if self.rank == 0:
            ...
            # Reference counting for multi-receiver RDMA sends: each TP rank
            # sends its own /send request, so the embedding must stay alive
            # until the last receiver has completed its transfer.
            self._send_ref_counts: Dict[str, int] = {}
            # Track which embeddings have been registered with Mooncake engine
            # to avoid double-register and ensure deregister on last /send.
            self._registered_embeddings: Dict[str, bool] = {}
    ...
    async def _send(
        ...
        if self.server_args.encoder_transfer_backend == "mooncake":
            ...
            # Register src GPU buffer once (first /send for this req_id registers;
            # subsequent ones skip since the buffer is already registered).
            req_id = mm_data.req_id
            if req_id not in self._registered_embeddings:
                self.engine.register(embedding.data_ptr(), embedding.nbytes)
                self._registered_embeddings[req_id] = True
            self.engine.transfer_sync(
                session_id, embedding.data_ptr(), buffer_address, embedding.nbytes
            )

            # Embedding lifetime is managed by _send_ref_counts in /send handler;
            # do NOT clear or deregister here so subsequent receivers can still transfer.
...
async def handle_send_request(request: dict):
    ...
    # Use reference counting to keep embedding alive for multi-receiver RDMA.
    # Each TP rank on the prefill side sends its own /send; the embedding is
    # released only after the last receiver completes its transfer.
    ...
    if encoder._send_ref_counts[req_id] <= 0:
        ...
        if mm_data is not None:
            # Deregister and release the embedding now that all receivers are done
            if req_id in encoder._registered_embeddings:
                if mm_data.embedding is not None:
                    encoder.engine.deregister(mm_data.embedding.data_ptr())
                encoder._registered_embeddings.pop(req_id, None)
            mm_data.embedding = None
        # Clean up async forward state if present
        if hasattr(encoder, "_forward_ready_events"):
            encoder._forward_ready_events.pop(req_id, None)
        if hasattr(encoder, "_forward_results"):
            encoder._forward_results.pop(req_id, None)
    return ORJSONResponse(content=None)
